...
import { Package, MapPin, Truck, Calendar, Clock } from 'lucide-react'
...
export default async function PortalPage() {
  ...
  // Get this transporter's existing bids to show "already bid" state
            ...
            return (
              <Card key={load.id} className={`relative overflow-hidden ${existingBid ? 'border-emerald-200' : ''}`}>
                ...
                <CardContent className="p-5">
                  {/* Route */}
                  <div className="flex items-start gap-2 mb-4 pr-20">
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium text-gray-700">{new Date(load.pickup_date).toLocaleDateString('en-IN')}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                  ...
                  {existingBid ? (
                    <div className="flex items-center justify-between">
                      <p className="text-sm text-emerald-700">Your bid: <span className="font-bold">₹{existingBid.bid_amount.toLocaleString('en-IN')}</span></p>
                      {!deadlinePassed && (
                        <BidModal loadId={load.id} existingBid={{ id: existingBid.id, amount: existingBid.bid_amount }} isEdit />
                      )}
                    </div>
                  ) : deadlinePassed ? (
