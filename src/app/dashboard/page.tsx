...
export default async function DashboardPage() {
  ...
  try {
    ...
    const [
      ...
      serviceClient.from('invoices').select('*, client:clients(name)').eq('business_id', bid),
      serviceClient.from('clients').select('id').eq('business_id', bid),
    ...
    function isPartial(inv: { status: string; paid_amount?: number | null }) {
      return inv.status !== 'paid' && Number(inv.paid_amount || 0) > 0
    }
    ...
    return (
      <div>
        <PageHeader
          title={`Good ${getGreeting()}, ${business.name || 'SIRPL'}`}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {partial && (
                          <p className="text-xs text-amber-600">
                            {formatCurrency(inv.paid_amount || 0)} paid · {formatCurrency(bal)} remaining
                          </p>
                        )}
                      </div>
                  ...
                  <div key={p.id} className="flex items-center justify-between py-3">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {p.invoice?.invoice_number || 'Payment'} · {p.payment_date ? formatDate(p.payment_date) : 'No date'}
                      </p>
                    </div>
                    <span className="text-sm font-semibold text-green-600">{formatCurrency(Number(p.amount || 0))}</span>
                  </div>
                ))}
              </div>
            </CardContent>
    ...
    console.error('DashboardPage render failed:', error)
    return (
      <div className="space-y-6">
        <PageHeader
          title="Invoice Dashboard"
          description="Dashboard loaded in safe mode because one of the live records could not be rendered."
        />

        <Card>
          <CardContent className="py-10">
            <p className="text-sm font-medium text-gray-900">The dashboard data is temporarily unavailable.</p>
            <p className="mt-2 text-sm text-gray-500">
              Try opening `Invoices`, `Clients`, or `Reports` from the sidebar. The main dashboard has been protected from crashing,
              and the error has been logged for follow-up.
            </p>
          </CardContent>
        </Card>
      </div>
